...
# 1. Replace the entire top section (Imports to the end of <style>)
new_top = """import streamlit as st
import pandas as pd
...
st.divider()
"""
content = re.sub(r'import streamlit as st.*?</style>\n""", unsafe_allow_html=True)', new_top, content, flags=re.DOTALL)

# 2. Replace the fetch_history function and Spotlight UI
# First remove old load_history and spotlight
content = re.sub(r'# ==========================================\n# 3\. DATA FETCHING.*?(?=# ==========================================\n# HEADER)', '', content, flags=re.DOTALL)

# Then insert new spotlight before tabs
new_spotlight = """
# Fetch history data safely
@st.cache_data(ttl=1)
def fetch_history():
    try:
...
# Now remove the old HEADER block since we moved it to the top
content = re.sub(r'# ==========================================\n# HEADER.*?(?=# ==========================================\n# TABS)', new_spotlight, content, flags=re.DOTALL)
